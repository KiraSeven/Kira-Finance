 ...
 * income.js
 * -----------------------------------------------------------------------
 * Page module Pemasukan: daftar transaksi + form tambah/edit + void/hapus.
 * -----------------------------------------------------------------------
 */

import { listIncome, createIncome, updateIncome, voidIncome, deleteIncome } from '../services/income.service.js';
...
import { DEFAULT_INCOME_CATEGORIES } from './config.js';
...
export async function render(container) {
  container.innerHTML = `
    <div class="page-header">
      <div class="page-header__text">
        <h1 class="text-2xl">Pemasukan</h1>
        <p>Catat seluruh pemasukan sekolah: SPP, dana BOS, sumbangan, dan lainnya.</p>
      </div>
      <div class="page-header__actions">
        <button type="button" class="btn btn--secondary" data-export-btn>⭳ Export Excel</button>
        <button type="button" class="btn btn--primary" data-add-btn>+ Tambah Pemasukan</button>
      </div>
    </div>
  ...
  async function refresh() {
    ...
    if (table) {
      ...
      table = createDataTable({
        ...
        rows: items,
        searchKeys: ['docNumber', 'category', 'description', 'payer'],
        emptyMessage: 'Belum ada pemasukan tercatat. Tambahkan transaksi pertama.',
  ...
  function openFormModal(existing) {
    const { formEl, validate, getValues } = createForm([
      ...
      {
        name: 'accountId', label: 'Akun Tujuan', type: 'select', half: true,
      ...
      },
      {
        ...
        value: existing?.category || DEFAULT_INCOME_CATEGORIES[0],
        options: DEFAULT_INCOME_CATEGORIES.map((c) => ({ value: c, label: c })),
        validate: required,
      },
      { name: 'payer', label: 'Dari (Pembayar)', value: existing?.payer, placeholder: 'Nama siswa / donatur / instansi' },
      ...
      onSubmit: async (values) => {
        try {
          if (existing) {
            await updateIncome(existing.id, { ...values, amount: Number(values.amount) });
            toastSuccess('Pemasukan diperbarui');
          } else {
            await createIncome(values);
            toastSuccess('Pemasukan ditambahkan');
          }
          closeModal();
          refresh();
        } catch (err) {
    ...
    if (accounts.length === 0) {
      openModal({
        ...
        body: '<p class="text-secondary">Tambahkan minimal satu akun kas/bank di menu Akun Kas & Bank sebelum mencatat pemasukan.</p>',
    ...
    openModal({
      title: existing ? `Edit Pemasukan · ${existing.docNumber}` : 'Tambah Pemasukan',
      body: formEl,
      footerButtons: [
  ...
  container.querySelector('[data-export-btn]').addEventListener('click', async (e) => {
    ...
    try {
      const items = await listIncome();
      await exportIncomeToExcel(items, accounts);
      toastSuccess('Export berhasil', 'File Excel pemasukan sudah diunduh.');
    } catch (err) {
...
async function exportIncomeToExcel(items, accounts) {
  const accountName = (id) => accounts.find((a) => a.id === id)?.name || '-';
  const today = todayInputValue();
  await exportToExcel({
    filename: `pemasukan-${today}`,
    sheets: [{
      name: 'Pemasukan',
      title: 'Kira Finance — Daftar Pemasukan',
...
function buildColumns(accounts) {
  ...
  return [
    ...
    { key: 'payer', label: 'Dari' },
    { key: 'accountId', label: 'Akun', render: (r) => accountName(r.accountId) },
    {
      key: 'status', label: 'Status',
      render: (r) => `<span class="badge badge--${r.status === 'void' ? 'neutral' : 'income'}"><span class="badge__dot"></span>${r.status === 'void' ? 'Void' : 'Posted'}</span>`,
    },
    { key: 'amount', label: 'Jumlah', align: 'right', render: (r) => `<span class="num text-income">${formatCurrency(r.amount)}</span>` },
  ];
}
